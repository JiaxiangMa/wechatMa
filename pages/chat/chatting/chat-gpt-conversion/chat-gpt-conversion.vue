<template>
	<view>
	<!-- 	<u-navbar  title="智能对话"  :background="{ background: '#f1f1f1'  }"  >
		</u-navbar> -->
		<u-notice-bar  padding="20rpx 24rpx"  type="success" height="80"
		...
		<view class="box-2">
			<view class="flex_col">
				...
				<button class="send clear" @tap="clear">清空</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		watch:{
			talkList(talkList){
				uni.setStorageSync('chatgpt_'+this.vuex_userInfo.id,JSON.stringify(talkList));
			}
		},
		methods: {
			// 获取历史消息
			...
			onMessage:function(content){
				...
				this.$u.api.toolApi.chatGpt(param).then(res => {
					if(res.code==200){
